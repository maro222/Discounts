...
object DiscountEngine {
  ...
  def qualifyExpiryDate(t: Transaction): Boolean = {
    ...
    ChronoUnit.DAYS.between(order_date, expiry_date) < 30
  }
...
object Main extends App {
  ...
  val dbUrl   = "jdbc:sqlite:orders.db"
  val csvPath = "H:/omar/ITI - Data Management/Scala/Discounts/src/main/scala/TRX1000.csv"

  // Step 1: Read raw lines from CSV  (side effect)
  val rawLines: List[String] =
    Infrastructure.readLines(csvPath) match {
      case Success(lines) =>
        Infrastructure.log("INFO", s"Read ${lines.length} lines")
        lines
      case Failure(e) =>
        Infrastructure.log("ERROR", s"Failed to read CSV: ${e.getMessage}")
        Nil
    }

  // Step 2: Parse into typed domain objects  (pure)
  val transactions: List[Transaction] =
    rawLines.tail
      .map(_.split(","))
      .flatMap(DiscountEngine.parseTransaction)

  // Step 3: Apply discount rules  (pure)
  val processed: List[ProcessedTransaction] =
    transactions.map(DiscountEngine.process)

  // Step 4: Write results to database  (side effect)
  ...
  // Step 5: Verify what was written  (side effect)
